...
# -*- coding: utf-8 -*-
"""
Compare Excel patterns with database schedule
Identify which pairs match patterns and which need correction
"""
...
# Week system: even weeks = RIGHT, odd weeks = LEFT
# Week 20 (Jan 12-18, 2026) = RIGHT
...
def main():
    print("=" * 80)
    print("PATTERN COMPARISON: Excel vs Database")
    print("=" * 80)
    ...
    for group, days in patterns.items():
        ...
        for day_name, pairs in days.items():
            for pair_num_str, pattern in pairs.items():
                ...
                if results['incorrect_pairs'] or results['missing_pairs']:
                    group_report['details'].append({
                        'day': day_name,
                        'pair': pair_num,
                        'pattern': pattern,
                        'incorrect': results['incorrect_pairs'][:3],  # Limit for brevity
                        'missing_count': len(results['missing_pairs'])
                    })
    ...
    # Print first few incorrect pairs
    if report['incorrect_to_delete']:
        ...
        for i, inc in enumerate(report['incorrect_to_delete'][:20]):
            ...
            print(f"{i+1:2}. {inc['group']:12} | {inc['day']:12} | Para {inc['pair']} | {date_str} ({inc.get('week_type', '?')})")
            print(f"    DB: {inc.get('db_discipline', 'N/A')[:40]}")
            print(f"    Expected: {inc.get('expected_discipline', 'EMPTY')}")
            print()

    # Save full report
    output_file = 'pattern_comparison_report.json'
    ...
    print("\nNext steps:")
    print("1. Review incorrect_to_delete list - these pairs have wrong disciplines")
    print("2. Review missing_to_create list - these pairs need to be generated")
    print("3. Run schedule correction script to fix issues")
