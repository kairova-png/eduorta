#!/usr/bin/env python
"""Generate English schedule for АиУ 3-25 and АиУ 3-25б using saved templates"""
...
def run_generation():
    with app.app_context():
        print("=" * 60)
        print("ENGLISH GENERATION FOR АиУ 3-25 AND АиУ 3-25б")
        print("=" * 60)
        ...
        group1 = Group.query.filter_by(name='АиУ 3-25').first()
        group2 = Group.query.filter_by(name='АиУ 3-25б').first()

        if not group1:
            print("ERROR: Group АиУ 3-25 not found!")
            return
        ...
        for w in workloads:
            ...
            print(f"  - Workload ID {w.id}: {w.group.name}")
